...
- [ ] Add explicit focus grouping support to the portfolio item model | AC: `PortfolioItem` accepts optional `focus?: boolean`, `SortOption` includes `focus`, a focus group is exported from the item data module, existing main category and project constellation group exports remain available
- [ ] Wire the focus option into sorting behavior | AC: the top control row renders a `focus` button, selecting `focus` gathers only the exported focus group, existing sort, category, and project constellation behavior remains unchanged
