...
- [x] Center presentation camera on the selected individual item | AC: opening or switching an individual item card targets/zooms the camera toward that item's scene position rather than the generic scene center; group/summary card camera behavior remains appropriate for the group overview and does not regress
...
- Individual item presentation stores the clicked rigid body's current world position as its camera target, with the item spawn position as a fallback. Group presentation intentionally keeps the origin-based overview target.
